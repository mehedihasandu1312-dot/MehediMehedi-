...
const Layout: React.FC<LayoutProps> = ({ user, setUser }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside 
        className={`fixed md:relative z-30 w-64 h-full bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-4 flex flex-col items-center border-b border-slate-100 bg-slate-50/50">
           <img 
            src={user.avatar || "https://picsum.photos/200/200"} 
            alt="Profile" 
            className="w-16 h-16 rounded-full border-2 border-indigo-100 mb-2"
          />
          <h3 className="font-semibold text-slate-800">{user.name}</h3>
          <span className="text-xs px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full font-medium mt-1">
            {user.role}
          </span>
        </div>
        ...
        <div className="p-4 border-t border-slate-100">
          <button 
            ...
            className="flex items-center w-full px-4 py-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
          >
